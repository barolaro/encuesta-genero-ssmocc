  ...
  return questions.flatMap((question) => {
    ...
    for (const response of responses) {
      ...
      if (Array.isArray(value)) {
        ...
        if (!clean) continue;
        const item =
          question.type === "text" ? "Comentarios recibidos" : "Respuesta";
        ...
        question.type === "multiple"
          ? responses.filter(
              (response) =>
                ...
                (response.responses[question.id] as unknown[]).length > 0,
            ).length
...
export async function GET(request: Request) {
  ...
  const selectedSections = selectedSurvey.sections as Array<{
    title: string;
    questions: Question[];
  }>;
  const allQuestions = selectedSections.length
    ? selectedSections.flatMap((section) => section.questions)
    : BASE_SURVEY_SECTIONS.flatMap((section) => section.questions);
  ...
  title(summary, `${selectedSurvey.title} · ${institution.institutionName}`, 5);
  ...
  Object.entries({
    ...
    const row = summary.addRow([
      label,
      item.value === null ? "Sin datos" : item.value / 100,
      item.base,
    ]);
  ...
  summary.addRow([
    `Los resultados segmentados se ocultan cuando la base es inferior a ${analysis.minimum} respuestas para proteger el anonimato. Los indicadores de percepción se expresan como porcentaje favorable; los indicadores inversos se ajustan al calcular el índice global.`,
  ]);
  summary.mergeCells(summary.rowCount, 1, summary.rowCount + 2, 5);
  summary.getCell(summary.rowCount - 2, 1).alignment = {
    wrapText: true,
    vertical: "top",
  };
  summary.columns = [
  ...
  if (analysis.responseCount < analysis.minimum)
    summary.addRow([
      "La base todavía es insuficiente para emitir conclusiones institucionales.",
    ]);
  else if (!weakest.length)
    summary.addRow([
      "No existen indicadores calculables con las respuestas disponibles.",
    ]);
  else
    weakest.forEach(
      (item, index) =>
  ...
  analysis.byUnit.forEach((item) => {
    const row = units.addRow([
      item.unit,
      item.responses,
      item.score === null ? "Protegido (<5)" : item.score / 100,
    ]);
    if (item.score !== null) row.getCell(3).numFmt = "0.0%";
  });
  units.columns = [{ width: 48 }, { width: 16 }, { width: 22 }];

  const matrix = workbook.addWorksheet("Matriz indicadores MINSAL", {
  ...
  analysis.indicators.forEach((item) => {
    const row = matrix.addRow([
      ...
      item.status,
      item.value === null
        ? `Base inferior a ${analysis.minimum} o sin respuestas válidas`
        : "Calculado automáticamente",
    ]);
    if (item.value !== null) row.getCell(5).numFmt = "0.0%";
  });
  ADMINISTRATIVE_INDICATORS.forEach((item) =>
    matrix.addRow([
      item.code,
  ...
  const distributions = workbook.addWorksheet("Distribución respuestas", {
    views: [{ state: "frozen", ySplit: 3 }],
  });
  ...
  REPORT_THEMES.forEach((theme) => {
    const sheet = workbook.addWorksheet(theme.sheetName, {
      views: [{ state: "frozen", ySplit: 4 }],
    });
    ...
    theme.indicatorCodes.forEach((code) => {
      ...
      const administrativeItem = ADMINISTRATIVE_INDICATORS.find(
        (item) => item.code === code,
      );
      if (surveyItem) {
        const row = sheet.addRow([
        ...
        sheet.addRow([
          administrativeItem.code,
          administrativeItem.name,
          "Administrativa",
          "",
          "",
          "Pendiente",
          "Requiere una base administrativa complementaria.",
        ]);
      }
    });
    const themeRows = allDistributions.filter((row) =>
      theme.questionIds.includes(row.questionId),
    );
    ...
    if (themeRows.length) addDistributionTable(sheet, startRow, themeRows);
      ...
      sheet.getCell(startRow, 1).value =
        "Este apartado no se calcula únicamente con respuestas de encuesta.";
  ...
  const questionMap = allQuestions.map((question) => ({
    id: question.id,
    title: question.title,
  }));
  const raw = workbook.addWorksheet("Base anonimizada", {
  ...
  raw.addRow([
    ...
    "Unidad",
    "Fecha",
    ...questionMap.map((question) => `${question.id} · ${question.title}`),
  ]);
  ...
  responses.forEach((response) =>
    raw.addRow([
      ...
      ...questionMap.map((question) =>
        safeValue(response.responses[question.id]),
      ),
    ]),
  );
  raw.getColumn(3).numFmt = "dd-mm-yyyy hh:mm";
  raw.columns.forEach((column, index) => {
    column.width = index < 3 ? [18, 38, 20][index] : 42;
  });
  raw.autoFilter = {
    from: { row: 1, column: 1 },
    to: { row: 1, column: raw.columnCount },
  };
  ...
  [
    [
      "Propósito",
      "Consolidar resultados de la encuesta y relacionarlos con los indicadores y complementos de equidad entregados como referencia.",
    ],
    [
      "Unidad de análisis",
      "Respuesta anónima. No se incluye RUT, correo, nombre ni otro identificador personal.",
    ],
    [
      "Regla de anonimato",
      `No se muestran resultados segmentados con menos de ${analysis.minimum} respuestas.`,
    ],
    [
      "Indicadores favorables",
      "Porcentaje de respuestas favorables según la definición de cada indicador.",
    ],
    [
      "Indicadores inversos",
      "Se invierte el sentido al incorporarlos al índice global.",
    ],
    [
      "Fuente encuesta",
      "Indicadores calculados directamente a partir de las 39 preguntas.",
    ],
    [
      "Fuente administrativa",
      "Indicadores que requieren nómina, remuneraciones, capacitación, movilidad, postulaciones o contrataciones.",
    ],
    [
      "Advertencia",
      "La ausencia de base administrativa no debe interpretarse como resultado cero ni como incumplimiento.",
    ],
