...
export async function GET() {
  ...
  return NextResponse.json({ surveys: allSurveys, responses, analysis: analyzeSurvey(responses), metrics: { total: responses.length, surveys: allSurveys.length, active: allSurveys.filter(s => s.status === "published").length, units: Object.keys(byUnit).length }, byUnit, byDay });
}
